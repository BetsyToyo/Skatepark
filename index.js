...
app.post("/login", async (req, res) => {
  ...
  const skater = skaters.find(
    (skater) => skater.email == email && skater.password == password
  );
  if (!skater) {
...
app.get("/datos", (req, res) => {
  res.render("datos", { layout: "datos", datos: skaterData });
});
...
app.put("/registro", async (req, res) => {
  ...
  res
    .status(respuesta.mensaje ? 500 : 201)
    .json(respuesta.mensaje ? respuesta.mensaje : respuesta);
});

//Modificacion de status en tabla administradorhgj
